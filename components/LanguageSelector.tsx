...
export function LanguageSelector({ currentLang, onLanguageChange, className = '', compact = false }: LanguageSelectorProps) {
  ...
  useEffect(() => {
    ...
    const savedLang = typeof window !== 'undefined' ? localStorage.getItem('user_lang') : null;

    let selectedCode = currentLang || (hasExplicitPathLang ? segments[0].toLowerCase() : undefined) || (hasExplicitQueryLang ? paramLang!.toLowerCase() : undefined) || savedLang || 'en';

    if (!currentLang && !hasExplicitPathLang && !hasExplicitQueryLang && !savedLang && typeof window !== 'undefined') {
      const detectedCode = detectBrowserLanguage();
      const detected = getLanguageByCode(detectedCode);
      selectedCode = detected.code;
    ...
    setActiveLang(getLanguageByCode(selectedCode).code);
  ...
  const handleSelect = (langCode: string) => {
    ...
    if (langCode === 'en') {
      ...
      navigateTo(`${safePath}${newParams.toString() ? `?${newParams.toString()}` : ''}`);
    } else if (safePath === '/' || safePath.startsWith('/category/') || safePath.startsWith('/error/') || safePath.startsWith('/brand/')) {
      navigateTo(safePath === '/' ? `/${langCode}` : `/${langCode}${safePath}`);
    } else {
      newParams.set('lang', langCode);
      navigateTo(`${safePath}?${newParams.toString()}`);
    }
  };
    ...
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      {detectedNotice && (
        <div className="absolute right-0 top-10 w-72 bg-gray-900 text-white p-3 rounded-xs shadow-xl border border-blue-500 z-50 text-xs font-sans">
          <div className="flex items-center justify-between"><span className="font-bold text-blue-400 text-[11px] uppercase tracking-wider">Language Detected</span><button onClick={() => setDetectedNotice(null)} className="text-gray-400 hover:text-white text-xs font-bold">✕</button></div>
          <p className="mt-1.5 text-gray-200 text-[11px] leading-snug">Your browser is set to <strong className="text-white">{detectedNotice.name}</strong> ({detectedNotice.flag}). Switch to view manuals in your language?</p>
          <div className="mt-2.5 flex items-center gap-2"><button type="button" onClick={() => handleSelect(detectedNotice.code)} className="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xs text-[11px]">{detectedNotice.flag} Switch to {detectedNotice.name}</button><button type="button" onClick={() => { setDetectedNotice(null); if (typeof window !== 'undefined') localStorage.setItem('user_lang', 'en'); }} className="px-2 py-1 text-gray-400 hover:text-gray-200 text-[11px]">Keep English</button></div>
        </div>
      )}
