...
export default function AnalyticsProvider() {
  ...
  useEffect(() => {
    ...
    if (!landingPath) { landingPath = `${pathname || '/'}${window.location.search}`; sessionStorage.setItem(landingKey, landingPath); }
    const referrer = document.referrer || null;
    const payload = {
      sessionId, path: `${pathname || '/'}${window.location.search}`, landingPath, referrer,
      utmSource: searchParams.get('utm_source'), utmMedium: searchParams.get('utm_medium'), utmCampaign: searchParams.get('utm_campaign'),
      deviceType: detectDevice(), browser: detectBrowser(), os: detectOS(), userAgent: navigator.userAgent,
      screenWidth: window.screen.width, screenHeight: window.screen.height, language: navigator.language,
      pageTitle: document.title, metadata: { timezone: Intl.DateTimeFormat().resolvedOptions().timeZone },
    };
    const controller = new AbortController(); const timer = window.setTimeout(() => controller.abort(), 2500);
    fetch('/api/analytics', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload), keepalive: true, signal: controller.signal }).catch(() => {}).finally(() => window.clearTimeout(timer));
