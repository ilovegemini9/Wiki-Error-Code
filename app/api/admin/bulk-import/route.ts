...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      const body = await req.json(); items = Array.isArray(body) ? body : [body];
    } else {
      const text = await req.text(); const lines = text.split(/\r?\n/).filter(l => l.trim());
      if (lines.length < 2) return NextResponse.json({ error: 'CSV file must include header and data rows' }, { status: 400 });
