...
export async function generateMetadata({ params }: LangCategoryPageProps) {
  ...
  return {
    title: `${category.name} Error Codes (${langObj.name}) - ErrorCodeWiki`,
    description: category.description || `Database of documented error codes, fault messages, and fixes for ${category.name} in ${langObj.name}.`,
  };
}
...
export default async function LangCategoryPage({ params }: LangCategoryPageProps) {
  ...
  // Get articles strictly for this category and language
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4">
        ...
        <Breadcrumbs items={[
          ...
          { label: `${category.name} (${langObj.flag} ${langObj.name})` }
        ]} />
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono font-bold bg-amber-400 text-gray-950 px-2 py-0.5 rounded-xs">
        ...
        <section className="my-6">
          {articles.length === 0 ? (
            <div className="p-8 text-center bg-gray-50 border border-gray-200 text-xs text-gray-600 rounded-xs">
              No articles documented in <strong>{langObj.name}</strong> under {category.name} yet.
            </div>
          ) : (
