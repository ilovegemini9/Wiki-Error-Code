...
export async function generateMetadata({ params }: LangBrandPageProps) {
  ...
  SUPPORTED_LANGUAGES.forEach(l => {
    languageAlternates[l.code] = l.code === 'en' ? `${siteUrl}/brand/${slug}` : `${siteUrl}/${l.code}/brand/${slug}`;
  });

  return {
    title: `${brand.name} Error Codes & Diagnostics - ${langObj.name}`,
    description: brand.description || `Database of error codes, fault messages, and solutions for ${brand.name} equipment in ${langObj.name}.`,
    alternates: {
      canonical: `${siteUrl}/${lang}/brand/${slug}`,
    ...
    <div className="min-h-screen bg-white text-gray-900 font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4">
        ...
        <Breadcrumbs items={[
          ...
          { label: `${brand.name} (${langObj.flag} ${langObj.code.toUpperCase()})` }
        ]} />

        {/* Brand Header */}
        <div className="my-4 pb-4 border-b border-gray-300 bg-[#f8f9fa] border p-6 rounded-xs">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-blue-900 text-white rounded-xs">
              <Tag className="w-5 h-5" />
            </div>
            <div>
              <h1 className="font-serif text-2xl font-bold text-gray-900">
                {brand.name} Fault Code Manuals ({langObj.name})
              </h1>
              <p className="text-xs text-gray-600 font-mono mt-0.5">
                {total} Documented {brand.name} Errors
              </p>
            </div>
          </div>
          ...
          <div className="mt-3 text-xs text-gray-600 font-mono flex items-center gap-2">
            <span className="font-bold text-gray-800">Supported Device Hardware:</span>
            <span>{brand.deviceTypes.join(', ')}</span>
          </div>
        </div>
        ...
        <div className="my-6 space-y-4">
          <h2 className="font-serif text-lg font-bold text-gray-900 border-b border-gray-200 pb-2">
            {brand.name} Articles & Diagnostic Guides ({langObj.name})
          </h2>

          {articles.length === 0 ? (
            <div className="p-8 text-center bg-gray-50 border border-gray-200 text-xs text-gray-600">
              No error codes listed under {brand.name} yet.
            </div>
          ) : (
            <div className="divide-y divide-gray-200 border border-gray-300 bg-white rounded-xs">
              {articles.map((art) => {
                const targetUrl = art.language === 'en' ? `/error/${art.slug}` : `/${art.language || lang}/error/${art.slug}`;
                return (
