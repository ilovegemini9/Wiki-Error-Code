...
import { getSupabaseArticleBySlug, getSupabaseArticles, getSupabaseBrandById, getSupabaseCategoryById, getSupabaseSettings, incrementSupabaseArticleViews } from '@/lib/supabase-db';
...
export async function generateMetadata({ params }: LangErrorPageProps) {
  const { lang, slug } = await params; const langObj = getLanguageByCode(lang); const article = await getSupabaseArticleBySlug(slug, lang);
...
export default async function LangErrorArticlePage({ params }: LangErrorPageProps) {
  ...
  const article = await getSupabaseArticleBySlug(slug, lang);
  if (!article) notFound();
  try { await incrementSupabaseArticleViews(article.id); } catch {}
  const [category, brand] = await Promise.all([getSupabaseCategoryById(article.categoryId), getSupabaseBrandById(article.brandId)]);
  const safeCategory = category || { name: article.categoryId, slug: article.categoryId };
  const safeBrand = brand || { name: article.brandId, slug: article.brandId };
  const { articles: relatedArticles } = await getSupabaseArticles({ status: 'published', categorySlug: safeCategory.slug, language: lang, limit: 5 });
  ...
  return <div className="min-h-screen bg-white text-gray-900 font-sans flex flex-col"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:jsonLdScript}} /><Navbar /><main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-12"><AdSlot placement="top" />
    <Breadcrumbs items={[{label:'Categories',href:`/${lang}`},{label:safeCategory.name,href:`/${lang}/category/${safeCategory.slug}`},{label:article.errorCode}]} />
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8"><article className="lg:col-span-3 space-y-8">
      ...
      {article.solutions?.length>0&&<section><h2 className="font-serif text-xl font-bold border-b border-gray-200 pb-1.5 flex items-center gap-2"><Check className="w-5 h-5 text-blue-700" />Step-by-Step Fix Instructions</h2><div className="mt-4 space-y-4">{article.solutions.map((sol,i)=><div key={i} className="border border-gray-300 rounded-xs bg-white p-5 space-y-3"><h3 className="font-serif font-bold flex items-center gap-2"><span className="px-2 py-0.5 bg-blue-700 text-white font-mono text-xs rounded-xs">Option {i+1}</span>{sol.title}</h3><p className="text-xs leading-relaxed">{sol.description}</p>{Array.isArray(sol.steps) && sol.steps.length>0&&<ol className="space-y-1.5 text-xs list-decimal list-inside">{sol.steps.map((step,j)=><li key={j}>{step}</li>)}</ol>}{sol.codeSnippet&&<div className="bg-gray-900 text-emerald-400 p-3.5 rounded-xs font-mono text-xs overflow-x-auto"><div className="text-[10px] text-gray-400 border-b border-gray-800 pb-1 mb-2 flex items-center gap-1"><Terminal className="w-3.5 h-3.5" />Terminal / Command Prompt</div><pre className="whitespace-pre-wrap">{sol.codeSnippet}</pre></div>}</div>)}</div></section>}
