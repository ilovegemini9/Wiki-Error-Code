...
export async function getSupabaseArticles(params?: { status?: 'draft' | 'published' | 'all'; search?: string; categorySlug?: string; brandSlug?: string; language?: string; limit?: number; offset?: number }): Promise<{ articles: Article[]; total: number }> {
  ...
  if (params?.categorySlug) { const { data, error } = await client.from('categories').select('id').eq('slug', params.categorySlug).maybeSingle(); if (error) throw error; if (data) query = query.eq('category_id', data.id); }
  if (params?.brandSlug) { const { data, error } = await client.from('brands').select('id').eq('slug', params.brandSlug).maybeSingle(); if (error) throw error; if (data) query = query.eq('brand_id', data.id); }
...
export async function saveSupabaseArticle(article: Partial<Article> & { errorCode: string; title: string }): Promise<Article> {
  ...
  const payload = { id: article.id || `art-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, error_code: article.errorCode.trim(), title: article.title,
    ...
    short_definition: article.shortDefinition || null, meaning: article.meaning || null, causes: article.causes || [], solutions: article.solutions || [],
    technical_explanation: article.technicalExplanation || null, faq: article.faq || [], schema_jsonld: article.schemaJsonLd || null, canonical_url: article.canonicalUrl || null,
    category_id: article.categoryId || null, brand_id: article.brandId || null, device_type: article.deviceType || null, language, keywords: article.keywords || [], tags: article.tags || [],
    featured_image: article.featuredImage || null, status: article.status || 'published', reading_time: article.readingTime || '3 min read', internal_links: article.internalLinks || [],
    created_at: article.createdAt || now, updated_at: now, scheduled_for: article.scheduledFor || null, views_count: article.viewsCount || 0, seo_score: article.seoScore ?? null,
...
export async function saveSupabaseAiGenerationLog(log: { id?: string; errorCode?: string; brand?: string; device?: string; model?: string; status: 'completed' | 'failed'; articleId?: string; promptText?: string; responseSummary?: string; usage?: unknown }): Promise<void> {
  const { error } = await requireClient().from('ai_generation_logs').upsert({
    id: log.id || `ai-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    error_code: log.errorCode || null, brand: log.brand || null, device: log.device || null, model: log.model || null,
    status: log.status, article_id: log.articleId || null, prompt_text: log.promptText || null, response_summary: log.responseSummary || null, usage: log.usage || null,
  }, { onConflict: 'id' });
...
export async function getSupabaseSettings(): Promise<Settings> { const { data, error } = await requireClient().from('global_settings').select('*').eq('id', 'global').maybeSingle(); if (error) throw error; if (!data) throw new Error('Global settings row not found. Run the Supabase migration first.'); return rowToSettings(data); }
