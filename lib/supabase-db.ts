 ...
 * Keep this module server-only: it uses the service-role client.
 */
...
export async function getSupabaseArticles(params?: {
  ...
  if (params?.categorySlug) {
    const { data: category, error } = await client.from('categories').select('id').eq('slug', params.categorySlug).maybeSingle();
    if (error) throw error;
    if (category) query = query.eq('category_id', category.id);
  }

  if (params?.brandSlug) {
    const { data: brand, error } = await client.from('brands').select('id').eq('slug', params.brandSlug).maybeSingle();
    if (error) throw error;
...
export async function getSupabaseSettings(): Promise<Settings> {
  const client = requireClient();
  const { data, error } = await client.from('global_settings').select('*').eq('id', 'global').maybeSingle();
  if (error) throw error;
...
export async function saveSupabaseSettings(settings: Partial<Settings>): Promise<Settings> {
  ...
  const payload = {
    ...
    default_ai_model: settings.defaultAiModel ?? null,
    language: settings.language ?? null,
    logo_url: settings.logoUrl ?? null,
    favicon_url: settings.faviconUrl ?? null,
    google_analytics_id: settings.googleAnalyticsId ?? null,
    google_search_console_tag: settings.googleSearchConsoleTag ?? null,
    google_search_console_meta: settings.googleSearchConsoleMeta ?? null,
    ads_txt_content: settings.adsTxtContent ?? null,
    robots_txt_content: settings.robotsTxtContent ?? null,
    default_language: settings.defaultLanguage ?? null,
    sitemap_settings: settings.sitemapSettings ?? null,
    automation_active: settings.automationActive ?? null,
    automation_interval_minutes: settings.automationIntervalMinutes ?? null,
    automation_languages: settings.automationLanguages ?? null,
    automation_publish_status: settings.automationPublishStatus ?? null,
    automation_model: settings.automationModel ?? null,
    last_automation_run_time: settings.lastAutomationRunTime ?? null,
    automation_count: settings.automationCount ?? null,
    automation_logs: settings.automationLogs ?? null,
  };
  const { data, error } = await client.from('global_settings').upsert(payload, { onConflict: 'id' }).select('*').single();
  if (error) throw error;
  return rowToSettings(data);
}
